package com.changseok.basic.repository;
...
import com.changseok.basic.entity.UserEntity;
...
// //
// description: JpaRepository<T, ID> - T: 해당 리포지토리에 사용될 엔터티 클래스, ID: 해당 엔터티
// 클래스에서 지정한 기본키 필드의 타입 //
public interface UserRepository extends JpaRepository<UserEntity, String> {
    ...
    // SELECT * FROM user WHWERE address_detail = '??' ORDER BY address DESC
    List<UserEntity> findByAddressDetailOrderByAddressDesc(String addressDetail);

    boolean existsByEmail(String email);

    // SELECT * FROM user WHERE email = '?' OR nickname = '?' OR tel_number = '?' ->
    // 레코드가 존재하는지
    ...
    // description: JPQL (Java Persistence Query Language) - SQL과 문법은 유사하지만 데이터베이스
    // 테이블이 아닌 엔터티 클래스 기준을 쿼리를 작성 //
